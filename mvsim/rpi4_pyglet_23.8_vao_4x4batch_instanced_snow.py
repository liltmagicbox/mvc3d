...
totalN=5000 # 66% speed up from 2000-4x4each.
...
def gldraw():
    ...
    for i in range(batchN):
        ...
        #for i in range(modelcol):
            #glUniform1i(idxID, i)
            #glDrawElements(GL_TRIANGLES, vaocart.size, GL_UNSIGNED_INT, None)
        glDrawElementsInstanced(GL_TRIANGLES, vaocart.size, GL_UNSIGNED_INT, None, modelcol)
        #see 70fps vs 107fps. faster!hahaha.
        #note: idx automatically by: gl_Position = Projection * View * Model[gl_InstanceID] * vec4(pos,1);
        #from now, this is not model-by draw..maybe. fixed vao, meshdata.
        #profiler says: costof uniform4x4(250)_2.0 ~= drawinstanced(250)_1.7.wow.        
    return 0
