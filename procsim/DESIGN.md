...
### "수신측이 로스를 모른다"는 절반만 맞다

UDP가 안 주는 것은 **자동 수리**와 **송신측 확인(ACK)**이지 '감지'가
아니다. 우리 프로토콜은 세 층에서 로스를 정확히 본다:

| 층 | 감지 수단 | 노출 |
|---|---|---|
| 프레임 | frame_id 단조증가 → 구멍 = 유실 개수 | `viewer.stat_skipped` |
| 청크 | offset 집합 + n_chunks → 어느 조각이 없는지 | `_try_fec`가 이걸로 복구 |
| 시간 | sim_time + 클록오프셋 → 상태가 몇 ms 묵었나 | `viewer.age_of()` |

감지한 다음의 응답이 설계 그 자체다: **싸게 고칠 수 있으면 고치고**(패리티
1청크), **아니면 버린다**(16ms 뒤 다음 프레임이 대체). 재전송을 '안' 하는
것이지 '못' 보는 게 아니다.

진짜 사각지대는 두 개고, 각각 처방이 있다:
1. **마지막 프레임 유실** — 시뮬이 멈춘 직후 프레임이 죽으면 대체할 다음
   프레임이 없다 → 고정 Hz 발행 유지(우리 기본) 또는 정지 시 최종 상태
   하트비트 재송신.
2. **이벤트류** — 위 결정의 단서 2: 신뢰 채널로 보낸다.
...
**최종 결정: 네트워크 구간의 기본 전송은 UDP+FEC로 통일한다.** 근거는 두
벤치의 결합이다 — 로컬(lan 프로파일)에서 UDP는 p95 17ms로 TCP와 동급이고,
원격에선 TCP의 RTO/head-of-line 동결 리스크가 구조적으로 없다. 즉 "로컬에서
빠르고, 같은 코드가 원격에서도 된다"가 성립한다. (TCP도 **로컬 전제로는**
충분함이 측정됐고 — wifi p95 23ms, ALVR은 TCP로 영상 250Mbps 실증 — 다만
그 결론은 로컬 전제에 묶이므로 단일 경로 기준에서는 UDP가 남는다.)

단서 두 개:
- **브라우저 뷰어는 raw UDP 불가** → 그 구간만 어댑터 교체: WebSocket(즉시
  가능, TCP지만 LAN에선 충분) → WebRTC DataChannel(UDP 의미론 유지,
  aiortc). 프레임 바이트는 동일하므로 시뮬 코드는 안 바뀐다.
- **제어·이벤트는 UDP 상태 스트림에 태우지 않는다.** 입력·점수·스폰은
  '상태'가 아니라 '사건'이라 드랍되면 그냥 소실이다(다음 프레임이 대체해
  주지 않는다). 신뢰 채널을 병행한다: TCP 컨트롤 소켓 하나, 또는 UDP 위
  seq+ack 재전송 미니레이어. ALVR이 control=TCP를 고정해 둔 이유가 이것.

수칙(전송 무관): 양끝 1칸 mailbox(백로그 금지), 무선 홉 1개(PC는 유선),
ws를 쓰는 구간은 `TCP_NODELAY`(안 끄면 소형 프레임이 ~40ms씩 묶인다).

```
토폴로지   PC(시뮬) ──유선── 공유기 ──무선── 헤드셋   ← 무선 홉은 딱 하나
라디오     5GHz(가능하면 6E/전용 SSID), 2.4GHz 금지(BT/전자레인지 간섭)
전송       UDP+FEC(udplink) + 양끝 latest-wins, f32 60Hz 그대로 (13.5Mbit)
           — 브라우저 뷰어 구간만 ws/WebRTC 어댑터, 제어는 신뢰 채널 병행
